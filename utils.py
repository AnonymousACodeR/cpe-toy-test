...
def create_unique_sibling_dir(base_dir_name):
    ...
    print(f"Created directory: {candidate}")
    return candidate
...
def plot_loss(history, args):
    ...
    save_path = create_unique_sibling_dir(save_path)
    ...
    ax.set_title(f"Comparison of models for {args.g_mode} task, N={args.N}, d={args.d}")
    
    # plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3, fontsize='small')
    plt.legend(fontsize=10)
    plt.tight_layout()

    plt.savefig(os.path.join(save_path, f"performance_graph_{title}.pdf"), dpi = 200, bbox_inches='tight')
